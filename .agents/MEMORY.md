...
- **Custom Cursor Snapping Refactor (200+ FPS)**: Implemented `cachedActiveRectRef` in `CustomCursor.tsx` to cache the bounding rect dimensions of snapped elements during hover, updating it only on scroll, resize, or hover changes; this completely eliminates `getBoundingClientRect()` calls inside the 120Hz/200Hz render loop, preventing layout thrashing and delivering a solid 200+ FPS cursor response; optimized scrollbar snap queries to only execute when the mouse coordinates are on the far right edge of the screen; introduced physical settling threshold locks (locking to target when within 0.05px) to completely eliminate lingering reticle oscillations.
- **Transform-Exempt Cursor Transitions**: Created a `.cursor-transition` class in `src/index.css` mapped to the custom cursor dot element; this transitions width, height, border-radius, background-color, and opacity while explicitly leaving `transform` transitions off, enabling the dot caret to snap instantly with zero input lag.
- **Separated Backdrop and Modal Pop Animations**: Refactored the Contribute, Registers, and Help overlays to animate the full-screen backdrop fade (opacity duration 0.15s) separately from the inner modal card's spring entry scaling (type spring, stiffness 420, damping 30); this bypasses full-screen layout redraw bottlenecks, yielding flawless 60+ FPS popup actions.
