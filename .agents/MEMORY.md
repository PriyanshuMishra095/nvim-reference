...
- **Framerate-Independent Cursor Physics**: Re-engineered the spring physics loop in `CustomCursor.tsx` to integrate a delta-time (`dt`) calculation normalized to 60fps; this guarantees the slow, fluid, and natural cursor feel is perfectly consistent regardless of monitor refresh rate (60Hz, 144Hz, 240Hz, or 360Hz), while utilizing the extra frames of high-Hz displays for ultra-fluid coordinate paths.
- **Separated Backdrop and Modal Pop Animations**: Refactored the Contribute, Registers, and Help overlays to animate the full-screen backdrop fade (opacity duration 0.15s) separately from the inner modal card's spring entry scaling; removed CPU-bound `backdrop-filter` blurs from the modal content cards themselves, replacing them with solid, high-opacity panels (`bg-white/98` / `bg-zinc-900/98`) to eliminate rendering/scale lag bottlenecks.
