...
- **Custom Cursor Snapping Refactor (200+ FPS)**: Implemented `cachedActiveRectRef` in `CustomCursor.tsx` to cache the bounding rect dimensions of snapped elements during hover, updating it only on scroll, resize, or hover changes; this completely eliminates `getBoundingClientRect()` calls inside the 120Hz/200Hz render loop, preventing layout thrashing and delivering a solid 200+ FPS cursor response; optimized scrollbar snap queries to only execute when the mouse coordinates are on the far right edge of the screen.
- **Visual Contrast & Hierarchy Polish**: Greyed out and resized code blocks, timeline step descriptions, and `vs_matrix` lists to `text-xs` (14px) and `text-zinc-500` / `text-zinc-400` to create a more sophisticated, balanced reading hierarchy; applied perfect layout alignment to bullet icons (`✕`, `✓` with `mt-0.5 shrink-0`) and step timeline badges (`top-[2px]`); made Registers info headers and code cell contents smaller (`text-xs`) and greyer; greyed out table headers (`text-zinc-400 dark:text-zinc-500`) and non-header cells (`text-zinc-500 dark:text-zinc-400`) to unify visual rhythm.
