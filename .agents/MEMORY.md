...
- **Global Typography Scale & Cohesive Colors**: Defined central typography scale variables (`--text-size-xs` to `--text-size-5xl`) in `index.css` mapped to the website's original design sizes (e.g. 12px for xs, 14px for sm, 16px for base, 18px for md, 20px for lg, 24px for xl, 30px for 2xl, etc.); overrode Tailwind font-size utilities and JIT classes (`.text-[10px]` to `.text-[13px]`) to bind them to these variables, preserving the layout proportions and headings styles; overrode default Tailwind neutral text color utilities to map semantically to theme variables (`--text-primary`, `--text-secondary`, `--text-muted`), ensuring consistent contrast and a highly cohesive layout in both dark and light modes.
