...
- **Typography Alignment with Hope Commit**: Completely reverted the custom typography scale (+2px sizes) and design tokens, restoring the exact typography scale, fonts, spacing, heights, and weights from the `hope` commit (`af05bf9b32`) using native Tailwind v4 text sizing; reverted element classes in `ChapterSection.tsx` and `VimStatusLine.tsx` to match the exact font size settings of the `hope` branch.
...
- **Framerate-Independent Cursor Physics**: Re-engineered the spring physics loop in `CustomCursor.tsx` to integrate a delta-time (`dt`) calculation normalized to 60fps; this guarantees the slow, fluid, and natural cursor feel is perfectly consistent regardless of monitor refresh rate (60Hz, 144Hz, 240Hz, or 360Hz), while utilizing the extra frames of high-Hz displays for ultra-fluid coordinate paths; split position follow physics (`posSpring=0.16` / `posFriction=0.40`) from snaps/dimensions physics (`dimSpring=0.28` / `dimFriction=0.40`) and border-radius physics (`rFriction=0.40`) with overdamped constants to completely eliminate size/shape bubble bounce.
- **Computed cursor style matching**: Integrated window.getComputedStyle lookup checking if `computedStyle.cursor === 'text' || computedStyle.cursor === 'vertical-text'` to automatically trigger custom text caret morphs on selectable headers, paragraphs, and descriptions across the landing page.
- **Static Class Name Transitions**: Removed all dynamic `dot.className` overrides from the physics loop, keeping the base classes static and using `classList` strictly for blinking states; this completely prevents stylesheet re-evaluations, letting text input caret transformations animate at 240fps.
- **Authentic Gemini Sparkle Logo Snap**: Replaced simple sparkles with an authentic Gemini branding logo (rotating large hypotrochoid star and bouncing small star); added `data-sparkles-btn` snap triggers to the Ask Neovim LLM Send button.
- **Interactive Code Block Vim Simulator**: Replaced static code render blocks in `ChapterSection.tsx` with an `InteractiveCodeBlock` buffer component; this component tracks focus and supports full Vim keyboard navigation (j/k: move cursor, i: insert mode text edits, y: yank line, :w: save simulated changes, :q: reset buffer, :%s/old/new/g: global replace).
- **High-FPS Magnetic Button Pull**: Disabled active CSS transitions (`transition: none`) on landing buttons during user dragging, restoring a 200+ FPS magnetic drag response.
- **Outer motion.div Exit Wrappers**: Converted standard backdrop wrapper containers to Framer Motion `motion.div` layers with key properties, enabling clean exit transitions for Help, Registers, and Contribute modals.
