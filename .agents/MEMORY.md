...
- **Scroll Parallax Orbs & Velocity Dampening**: Aurora background orbs velocity has been dampened to an ultra-slow drift (`vx/vy` random between -0.02 and 0.02); added a depth-aware scroll-parallax translation (`scrollParallaxY` based on `scrollY * 0.08 * depth`) to provide a cinematic floating feel behind text content.
...
- **Ask Neovim LLM Chat Refinements**: Changed LLM branding references from Gemini to "Ask Neovim LLM"; implemented context-aware chat input placeholder mapping ("Ask Neovim LLM..." if chat messages is empty, "Ask a follow-up question..." if conversation exists), resolving help index vs command box input inconsistencies.
