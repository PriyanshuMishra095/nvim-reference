...
### [2026-07-09T02:00:00+05:30] Bug-fix pass on unified-overhaul (user-reported round)
- **Mechanical**: AGENTS.md Git&Security bullet → "Public repo is curated, not mirrored"; `origin` remote repointed to nvim-reference.git (repo renamed on GitHub); repo links updated to PriyanshuMishra095/nvim-reference in App.tsx (Contribute modal href+label), project_continuation_document.md, project_summary_report for llms.md.
- **CustomCursor.tsx**: recompute hover shape on `click` (capture) + delayed passes and on `nvim:cursor-refresh` event → checklist card flips to red X the instant it's toggled; modals opening over a locked element reset the cursor without needing a mousemove. Fixes contribute-open stuck-morph and checklist-X-lag.
- **App.tsx**: removed the idle ghost-caret easter egg (confusing green caret crossing screen); toggleTheme rewritten — reads live data-theme (rapid clicks no longer cancel out), guards overlapping View Transitions (themeBusyRef), sets `data-theme-switching` to suppress the global color transition during the swap (kills theme-change lag), VT reveal shortened 1.2s→0.55s; hero nvim:// prefix → neon-teal; dispatch nvim:cursor-refresh when palette/wizard/contribute open.
- **index.css**: `:root[data-theme-switching] *` snaps transitions to 0 during theme swap; ghost numeral bounded to title band (top-anchored, smaller, no clip, never behind code blocks); removed will-change from 22 progress bars; removed dead ghost-caret keyframes.
- **CommandPalette.tsx**: all key handling moved to a window capture listener (Esc/arrows/Enter work after clicking inside the panel); manual rect-delta autoscroll; list + preview given their own max-h so the list actually overflows and the active row stays visible.
- **VimStatusLine.tsx**: help modal gets a back button + topic name in header + min-h-[460px] for consistent sizing + keyed cross-fade between topics; "*registers-api*" renamed "*registers*"; held j/k use instant small steps on key-repeat (no more flicker), single tap stays smooth; statusline backdrop-blur removed (scroll repaint cost). Added :wizard command earlier; :Tutor resets mode.
